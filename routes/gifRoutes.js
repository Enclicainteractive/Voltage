...
const pickFormat = (file, size = 'sm', format) => {
  ...
  const formats = format ? [format, ...CONTENT_TYPES.gifs.formats] : CONTENT_TYPES.gifs.formats
...
const createTrendingHandler = (type) => async (req, res) => {
  ...
  try {
    ...
    res.json({
      results: Array.isArray(data?.data?.data) ? data.data.data.map(item => normalizeItem(item, contentType)) : [],
      next: data?.data?.has_next ? String((data?.data?.current_page || page) + 1) : null,
      source: 'klipy',
      tracking_enabled: enableTracking
    })
...
const createShareHandler = (type) => async (req, res) => {
  ...
  try {
    const config = CONTENT_TYPES[contentType]
    const customerId = getCustomerId(req, true)
